...
RAYGUIDEF bool GuiWindowBox(Rectangle bounds, const char *title)
{
    ...
    Rectangle statusBar = { (float) bounds.x, (float) bounds.y, (float) bounds.width, (float) statusBarHeight };
    ...
    if (state == GUI_STATE_PRESSED)
    {
        ...
        if (editMode && ((framesCounter/20)%2 == 0))
        {
            ...
            Rectangle cursor = {
                ...
                1, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) + (float) GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING)
            };
